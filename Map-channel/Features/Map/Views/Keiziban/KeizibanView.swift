...
// 新規投稿ウィンドウ（当座のプレースホルダ。isPresented バインディングに対応）
struct KZBNewPostView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // ヘッダーバー（見た目は簡易。UI崩しを避けるため配色だけ既存トーンに合わせています）
            HStack {
                Text("新規投稿")
            ...
            .clipShape(KZBRoundedRect(topLeft: 16, topRight: 16, bottomLeft: 0, bottomRight: 0))
            
            // コンテンツ（最小のフォーム。後で本実装へ差し替え可）
            VStack(alignment: .leading, spacing: 12) {
                Text("※プレースホルダです。後で本実装に差し替えてください。")
                    .font(.footnote)
                    .foregroundColor(.secondary)
                TextField("タイトル", text: .constant(""))
                    .textFieldStyle(.roundedBorder)
                TextField("本文", text: .constant(""))
                    .textFieldStyle(.roundedBorder)
                HStack {
                    Spacer()
                    Button {
                        ...
                        Text("投稿する")
                            ...
                            .padding(.horizontal, 18)
                            .padding(.vertical, 10)
                            .background(RoundedRectangle(cornerRadius: 12).fill(Color.kzb("#2E4A8C")))
                    }
                }
            }
            .padding(16)
            .background(Color(.secondarySystemBackground))
            .clipShape(KZBRoundedRect(topLeft: 0, topRight: 0, bottomLeft: 16, bottomRight: 16))
        }
        .background(
